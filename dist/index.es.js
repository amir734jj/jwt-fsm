...
function () {
  ...
  JwtFsm.validate = function (token) {
    return moment.duration(moment(JwtFsm.tokenExpires(token)).diff(moment())).asMilliseconds() > 0;
  };
  /**
  ...
  JwtFsm.prototype.scheduleRenewal = function () {
    ...
    var renewal = moment.duration(moment(JwtFsm.tokenExpires(this.token())).diff(moment())).subtract(this._options.renewal, 'minutes').asMilliseconds();
